...
class GraphArea extends Component{
  ...
  updateData() {
    ...
    if(this.props.data.length !== 0) {
      let sect  = this.props.sectors[sectorIndx % this.props.sectorLength]
      if(sect === 'All')
        ...
        d3.selectAll('.sectorName')
          .html(`${sect} industry`)
          let avgPts = []
      this.state.topics.forEach((el,j) => {
      ...
      this.state.topics.forEach((el,i) => {
        this.props.data[sect]['frequency'][el].forEach((d,j) => {
  ...
  render(){ 
    if(this.props.data === 0)
      ...
      let txt = this.state.topics.map((d,i) => {
        return (
          <text 
            key={i}
      ...
      let txt1 = this.state.topics.map((d,i) => {
        return (
          <text 
            key={i}
            className='headerText'
      ...
      let labelTxt = this.state.value.map((d,i) => {
        return (
          <text 
            key={i}
            className='labelText'
      ...
      let polyLines = this.props.data[this.props.sectors[0]]['data'].map((d,i) => {
        let points = []
        this.state.topics.forEach((el,j) => {
          let xPos = this.widthScale(el);
      ...
      this.state.topics.forEach((d, i) => {
        this.props.data[this.props.sectors[0]]['frequency'][d].forEach((el,j) => {
